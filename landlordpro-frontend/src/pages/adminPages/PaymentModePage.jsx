...
// Use STRING values for Select options
...
const PaymentModePage = () => {
  ...
  const [editData, setEditData] = useState({ 
    code: '', 
    displayName: '', 
    requiresProof: 'false',  // default as string
    description: '' 
  });
  ...
  // Normalize incoming requiresProof to boolean
  ...
  const fetchPaymentModes = async (pageNumber = 1, search = '') => {
    try {
      ...
      const modesArray = Array.isArray(res)
        ? res
        : Array.isArray(res.paymentModes)
        ? res.paymentModes
        : [];

      const modes = modesArray.map(m => ({
        ...m,
        requiresProof: normalizeRequiresProof(m.requiresProof)
      }));
  ...
  const handleEditClick = (mode) => {
    setEditData({
      ...
      requiresProof: mode.requiresProof ? 'true' : 'false',  // convert to string
  ...
  // Create or update payment mode - FIXED VERSION
  const handleSubmit = async () => {
    const { code, displayName, requiresProof, description } = editData;
    if (!code?.trim() || !displayName?.trim()) {
      return showError('Code and Display Name are required.');
    }
    ...
    const payload = {
      ...
      requiresProof: requiresProof === 'true',  // Compare with STRING 'true'
  ...
  const handleDelete = async (mode) => {
    ...
    try {
      ...
      showInfo('Payment mode deleted successfully.');
      refreshList();
    } catch (err) {
      showError(err?.message || 'Failed to delete payment mode');
    }
  };
  ...
  const handleRestore = async (mode) => {
    try {
      ...
      showSuccess('Payment mode restored successfully.');
      refreshList();
    } catch (err) {
      showError(err?.message || 'Failed to restore payment mode');
    }
  };

  // Filtered payment modes for search
  ...
  return (
    <div className="space-y-6 pt-12 px-3 sm:px-6">
              ...
              {filteredModes.map(mode => (
                <Card key={mode.id} className="bg-white p-4 rounded-xl shadow-md border border-gray-100 flex flex-col gap-2">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 mt-2">
                    ...
                    {mode.deleted_at ? (
                      <Button 
                        className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-md text-xs flex-1 flex items-center justify-center gap-1" 
                        onClick={() => handleRestore(mode)}
                      >
                        <FiRefreshCcw className="text-sm" /> Restore
                      </Button>
                      ...
                      <Button 
                        className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-md text-xs flex-1 flex items-center justify-center gap-1" 
                        onClick={() => handleDelete(mode)}
                      >
                        <FiTrash className="text-sm" /> Delete
                      </Button>
      ...
      {modalOpen && (
        <Modal 
          title={selectedMode ? 'Edit Payment Mode' : 'Add New Payment Mode'} 
        ...
        >
          <div className="space-y-4">
            ...
            <Select
                ...
                value={requiresProofOptions.find(opt => opt.value === editData.requiresProof)}
                onChange={(selected) => {
                  if (selected) {
                    setEditData(prev => ({
                      ...prev,
                      requiresProof: selected.value 
                    }));
                  }
                }}
              />
