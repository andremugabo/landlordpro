...
const PropertyPage = () => {
  ...
  const fetchProperties = async (pageNumber = 1) => {
    try {
      ...
      setTotalPages(1);
      setPage(1);
      const message = err.message || err?.message || 'Failed to fetch properties';
      showError(message); // Always show toast on fetch failure
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditClick = (property) => {
    ...
    setEditData({ name: property.name, location: property.location, description: property.description || '' });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedProperty) {
        await updateProperty(selectedProperty.id, { name, location, description });
      ...
      const message = err?.message || 'Failed to save property';
      showError(message); // Always show toast on error
    }
  };
  ...
  const handleDelete = async (property) => {
    ...
    try {
      ...
      showError(message); // Always show toast on delete error
    }
  };
